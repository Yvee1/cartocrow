...
TEST_CASE("Creating polar points") {
	...
	PolarPoint p2(2, 0.5 * std::numbers::pi);
	CHECK(p2.r() == Approx(2));
	CHECK(p2.phi() == Approx(0.5 * std::numbers::pi));
	PolarPoint p3(3, 1.5 * std::numbers::pi);
	CHECK(p3.r() == Approx(3));
	CHECK(p3.phi() == Approx(-0.5 * std::numbers::pi));
	...
	CHECK(p5.r() == Approx(1));
	CHECK(p5.phi() == Approx(0.5 * std::numbers::pi));
	PolarPoint p6(Point<Inexact>(-1, 0));
	CHECK(p6.r() == Approx(1));
	CHECK(p6.phi() == Approx(-std::numbers::pi));
	PolarPoint p7(Point<Inexact>(0, -1));
	CHECK(p7.r() == Approx(1));
	CHECK(p7.phi() == Approx(-0.5 * std::numbers::pi));
}

TEST_CASE("Translating polar points") {
	PolarPoint p1(1, 0.5 * std::numbers::pi);
	PolarPoint p2(p1, Vector<Inexact>(1, 0));
	CHECK(p2.r() == Approx(std::sqrt(2)));
	CHECK(p2.phi() == Approx(0.25 * std::numbers::pi));
}
